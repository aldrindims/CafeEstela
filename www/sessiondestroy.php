	...
	foreach($_SESSION["shopping_cart"] as $keys => $values){
			if($values["product_type"] == "drink"){	
				...
				elseif($values['product_size'] == "16oz"){					
					$sql1 = "SELECT * FROM tbl_prod_ingr2 ORDER BY `tbl_prod_ingr2`.`id` ASC";
					$result=mysqli_query($connect, $sql1);
					while($row = $result->fetch_assoc()){
						if($row['id'] == $values['product_id']){
							$min_array = array(
			...
			while($row2 = $result2->fetch_assoc()){
				if($row2['meal_id'] == $values['product_id']){
					...
					'product_ingr8'				=>	$row2['ingr8_id'],
					'product_ingr9'				=>	$row2['ingr9_id'],
					'product_ingr10'			=>	$row2['ingr10_id'],
					'product_ingr11'			=>	$row2['ingr11_id'],
					'product_ingr12'			=>	$row2['ingr12_id'],
					...
					'product_ingrm8'			=>	$row2["ingrm8"],
					'product_ingrm9'			=>	$row2["ingrm9"],
					'product_ingrm10'			=>	$row2["ingrm10"],
					'product_ingrm11'			=>	$row2["ingrm11"],
					'product_ingrm12'			=>	$row2["ingrm12"],
				);
...
foreach($_SESSION["shopping_cart"] as $keys => $value){
		$name = $value["product_name"];
		$first = $value["product_price"];
		$second = $value["product_quantity"];
		$price = $first * $second; ////////////binago ko to 04/28/2017
		$date = date("Y-m-d");
		$size = $value["product_size"];
		if($size == '18oz'){
			$size= '';
		}
		$sql = "INSERT INTO `tbl_report` (`id`, `trans_no`, `prods`,`size`, `qty`, `date`, `Amount`) VALUES ('$increment', '$increment', '$name','$size' , '$second', '$date', $price);";
			mysqli_query($connect, $sql);
